...
class CustomTableModel(QtCore.QAbstractTableModel):
    ...
    def __init__(self, data=None):
        super().__init__()
        self.user_data = data if data is not None else []  # 기본값으로 빈 리스트를 할당
        self.columns = list(self.user_data[0].keys()) if self.user_data else []  # 데이터가 없으면 빈 리스트
        self._data = self.user_data  # 데이터를 내부 변수로 저장

    def rowCount(self, *args, **kwargs):
        return len(self.user_data)  # 데이터 행(row)의 개수를 반환

    def columnCount(self, *args, **kwargs):
        return len(self.columns)  # 데이터 열(column)의 개수를 반환
    ...
    def update_data(self, new_data):
        ...
        self.beginResetModel()
        self.user_data = new_data  # 새로운 데이터 저장
        self.columns = list(self.user_data[0].keys()) if self.user_data else []  # 새로운 컬럼명 설정
        self.endResetModel()
    ...
    def set_data(self, new_data):
        ...
        데이터를 설정하고, 갱신하는 메서드
        """
        self.update_data(new_data)  # update_data 메서드를 호출하여 데이터 갱신
        self.modelReset.emit()  # 테이블 모델이 갱신되었음을 알리는 신호를 보내어 UI에 반영되도록 함
