...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def apply_filter(self):
        ...
        # 페이지네이션을 위한 skip과 limit 값 설정 (예시)
        skip = 0  # 예시로 첫 페이지
        limit = 10  # 예시로 10개씩 제한

        # 필터와 정렬, 페이지네이션 정보를 get_all_assets에 전달
        ...
        print(f"Found assets: {assets}")  # 터미널에 디버깅 출력을 확인

        # 테이블 모델 업데이트
        if assets:
            self.model.set_data(assets)  # CustomTableModel의 set_data 메서드를 통해 모델 데이터를 업데이트
            self.model.layoutChanged.emit()  # 테이블 뷰를 갱신하기 위해 layoutChanged 신호를 보내는 것

        else:
            print("No assets found with the applied filter.")
